...
class DashboardController extends Controller
{
    ...
    public function reportclient()
    {
        ...
        $fpdf->Cell(280,12,'LISTE DES CLIENTS',1,0,'C');
        $fpdf->Ln();
        $fpdf->Cell(10,6,'ID',1,0,'C');
        $fpdf->Cell(80,6,'NOMS',1,0,'C');
        $fpdf->Cell(60,6,'ADRESSE',1,0,'C');

        $fpdf->Cell(40,6,'CONTACT',1,0,'C');
        $fpdf->Cell(90,6,'ADRESSE MAIL',1,0,'C');
        $fpdf->Ln();
        foreach($listeclient as $list){
            ...
            $fpdf->Cell(60,8,$list->adresse,1,0,'C');

            $fpdf->Cell(40,8,$list->telephone,1,0,'C');
            $fpdf->Cell(90,8,$list->mail,1,0,'C');
            $fpdf->Ln();
        }
        $fpdf->Output();
        exit;
